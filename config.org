...
#+begin_src emacs-lisp
 (use-package mood-line
  :ensure t                          ; Install from MELPA if needed
  :config
  (mood-line-mode 1))                 ; Enable globally
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun my-dired-sort ()
    ...
    'name' uses natural/human-readable sorting (v/V flag in ls)."
    (interactive)
    (let ((sort-options
  ...
  (add-hook 'dired-mode-hook
            (lambda ()
              (evil-define-key 'normal dired-mode-map (kbd "?") #'dired-get-size)))
...
#+end_src
** Scratch Buffer
Scratch Buffer defaults to org-mode
#+begin_src emacs-lisp
...
(defun zoxide (q)
  "Fuzzy jump using zoxide, supports space-separated terms."
  (interactive "sZoxide: ")
  (let* ((bin (executable-find "zoxide"))
         (args (cons "query" (split-string q " +" t))))
    (unless bin (error "Install zoxide"))
    (with-temp-buffer
      (if (= 0 (apply #'call-process bin nil t nil args))
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-c C-i C-u") #'insert-char)
...
#+begin_src emacs-lisp
  ...
  (evil-global-set-key 'normal (kbd "<leader>fl") #'consult-line)
  ...
  (evil-global-set-key 'normal (kbd "<leader>fy") #'consult-yank-from-kill-ring)
  ...
  (evil-global-set-key 'normal (kbd "<leader>wk") #'kill-buffer-and-window)
  ...
  (evil-global-set-key 'normal (kbd "<leader>sb") #'save-buffer)
  (evil-global-set-key 'normal (kbd "<leader>dj") #'dired-jump)
  (evil-global-set-key 'normal (kbd "<leader>cd") #'zoxide)
  ...
  (evil-global-set-key 'normal (kbd "<leader>qq") #'kill-buffer-and-window)
...
#+begin_src emacs-lisp
  (use-package vundo
    ...
    (custom-set-faces
     '(vundo-node ((t (:foreground "#cecece"))))
     '(vundo-stem ((t (:foreground "#cecece"))))
     '(vundo-highlight ((t (:foreground "#2adcdc")))))
